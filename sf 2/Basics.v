...
match n with
|B => O
|T n' => mult 2 (toUnary n')
|X n' => S (mult 2 (toUnary n'))
end.

Example b1 := B.
Compute toUnary b1.

Example incb1 : toUnary(increment b1) = 1.
Proof.
compute.
reflexivity.
Qed.
...
Example incb2 : toUnary(increment b2) = 2.
Proof.
compute.
reflexivity.
Qed.
...
Example incb3 : toUnary(increment b3) = 3.
Proof.
compute.
reflexivity.
Qed.
...
Example testMinus: minus' (S (S (S (S O)))) (S (S (S O))) = 1.
Proof.
compute.
reflexivity.
Qed.
