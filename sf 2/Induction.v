...
(* Destruct and induction both separate out into cases but indution also
   includes the induction hypothesis (IHn) *)
...
Fixpoint toBinary (n: nat): bin :=
match n with
|O => B
|S n' => increment (toBinary n')
end.

Theorem toBin: forall (n: nat), n = toUnary( toBinary n).
Proof.
intros.
induction n.
Case ("n = 0").
...
Case ("n = S n").
simpl.
Lemma binInc: forall (n: bin),toUnary(increment(n)) = 1 + toUnary(n).
 Proof.
 intros.
 induction n.
 Case ("n = B").
...
Theorem plus_comm_informal: forall n m: nat,
  n+ m = m + n.
Proof.
 intros. induction m as [|m']. simpl. rewrite -> plus_0_r.
 reflexivity. simpl. rewrite <- plus_n_Sm. rewrite <- IHm'. 
 reflexivity.
Qed.
...
(** Theorem: Addition is commutative.
    ...
    - First, suppose m = 0.
      n + 0 = 0 + n
      n = n following the definitin of [+]
    - Next, suppose if [m = S m'], where
      n + m' = m' + n
      We must show that 
      n + S m' = S m' + n
      This follows directly from the definiton of [+] that
      n + S m' = S (m' + n)
      S (n + m') = S (m' + n)
      Thus, through the induction hypothesis
      S (n + m') = S (n + m')
    
*)


Theorem beq_nat_refl_infornal : forall n : nat, 
  true = beq_nat n n.
Proof.
intros. 
induction n as [|n'].
...
(** Write an informal proof of the following theorem, using the
    ...
    - First, suppose n = 0.
      true = beq_nat 0 0
      true = true following the definiton of beq_nat
    - Next, suppose if [n = S n'], where true = beq_nat n' n'
      We must show that
      beq_nat n' n' = beq_nat (S n') (S n')
      By definition of beq_nat
      beq_nat n' n' = beq_nat n' n'
      Thus we can see through the induction hypothesis
      true = true
[]
 *)
